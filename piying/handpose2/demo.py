...
from geom_puppet import GeomPuppet, Vec2
...
ORIGIN_MAX_SPEED_X = 24000.0
ORIGIN_MAX_SPEED_Y = 10000.0
...
def _load_shadow_surface():
    """Load semi-transparent shadow surface from vision_resources/inside_cave.png at 80% opacity."""
    try:
        png_path = os.path.join(PROJECT_DIR, 'vision_resources', 'inside_cave.png')
        shadow = pygame.image.load(png_path).convert_alpha()
        shadow = pygame.transform.smoothscale(shadow, (WIN_W, WIN_H))
        shadow.set_alpha(int(0.8 * 255))
        return shadow
    except Exception:
        return None


def Hand_control_puppet(hand, frame, bones, bone_state,
                        origin_x, origin_y, origin_vx, origin_vy,
                        origin_offset_y, flip,
                        last_ox, last_oy, prev_ovx, prev_ovy,
                        clock, G,
                        neutral_frame_request,
                        locked_active, locked_mapping, rope_len_map,
                        trunk_waist_drive, trunk_waist_vel):
    h, w = frame.shape[:2]
    def LM(lm):
    ...
    if not neutral_frame_request:
        ...
        bone_state[b_head.name]['target'] = bone_state[b_head.name]['rest'] + max(-10.0, min(10.0, orient_deg))
        ...
        if locked_active and locked_mapping and b_left_leg and b_right_leg:
            ...
            tips_by_id_local = {
                4: LM(hand.landmark[4]),
                8: LM(hand.landmark[8]),
                12: LM(hand.landmark[12]),
                16: LM(hand.landmark[16]),
                20: LM(hand.landmark[20]),
            }
            try:
                if fid_left is not None:
                    ...
                    if ft is not None:
                        ...
                        upward_pull = max(0, ly - rope_end_y)
            ...
            try:
                ...
                pass
        b_body = next(b for b in bones if b.name == 'body')
        bone_state[b_body.name]['target'] = bone_state[b_body.name]['rest'] + body_delta
        try:
            ...
            drive_target = body_delta + hip_extra_tension
            ...
            pass
    MIDDLE_TIP = 12
    mt = hand.landmark[MIDDLE_TIP]
    txo = WIN_W - int(mt.x * WIN_W)
    tyo = int(mt.y * WIN_H) + origin_offset_y
    ...
    origin_vx += ax_o * dt_o
    origin_vy += ay_o * dt_o
    origin_x += origin_vx * dt_o
    origin_y += origin_vy * dt_o
    prev_ovx, prev_ovy = origin_x - last_ox, origin_y - last_oy
    last_ox, last_oy = origin_x, origin_y
    return (bone_state, origin_x, origin_y, origin_vx, origin_vy,
            trunk_waist_drive, trunk_waist_vel, last_ox, last_oy, prev_ovx, prev_ovy)

def puppet_match(bones, ghost_bones, ghost_origin, origin_x, origin_y, flip,
                 MATCH_THRESH_BODY, MATCH_THRESH_OTHERS, locked_active):
    ...
    for b in bones:
        ...
        thr = MATCH_THRESH_BODY if b.name == 'body' else MATCH_THRESH_OTHERS
        total_n += (math.sqrt(d2_j) / thr) + (math.sqrt(d2_t) / thr)
        count += 2
    avg_n = (total_n / count) if count > 0 else 1e9
    return avg_n <= 1.0

def collide(obj, bones, origin_x, origin_y, flip,
            obj_x, ground_y,
            obj_shake, obj_vel, obj_K, obj_C,
            obj_ang, obj_ang_vel, obj_ang_target, obj_ang_K, obj_ang_C,
            obj_hit_ang,
            clock):
    dt_phys = 1.0 / max(1.0, clock.get_fps() or 30.0)
    obj_vel += (-obj_K * obj_shake - obj_C * obj_vel) * dt_phys
    obj_shake += obj_vel * dt_phys
    obj_ang_vel += (-obj_ang_K * (obj_ang - obj_ang_target) - obj_ang_C * obj_ang_vel) * dt_phys
    obj_ang += obj_ang_vel * dt_phys
    def tip_screen_world(b):
        pos = b.world_pos()
        tip = b.tip_offset()
    ...
    oi = obj['image']
    sc = obj['scale']
    tw = max(1, int(oi.get_width() * sc))
    th = max(1, int(oi.get_height() * sc))
    base_rect = pygame.Rect(0, 0, tw, th)
    base_rect.midbottom = (obj_x, ground_y + int(obj_shake))
    shapes = obj['shapes'] if obj['shapes'] else [{'x': 0.0, 'y': 0.0, 'w': float(oi.get_width()), 'h': float(oi.get_height()), 'r': 12}]
    any_hit = False
    for b in bones:
        tx, ty = tip_screen_world(b)
        for sh in shapes:
            rx = base_rect.left + int(sh.get('x', 0.0) * sc)
            ry = base_rect.top + int(sh.get('y', 0.0) * sc)
            rw = int(sh.get('w', 0.0) * sc)
            rh = int(sh.get('h', 0.0) * sc)
            r = pygame.Rect(rx, ry, rw, rh)
            if r.collidepoint(tx, ty):
                any_hit = True
    if any_hit:
        angle_sign = (1.0 if origin_x > obj_x else -1.0)
        obj_ang_target = angle_sign * obj_hit_ang
    else:
        obj_ang_target = 0.0
    return (obj_shake, obj_vel, obj_ang, obj_ang_vel, obj_ang_target)
def main():
    pygame.init()
    ...
    origin_x, origin_y = WIN_W // 2, WIN_H // 2
    origin_vy = 0.0
    ...
    ghost_origin = [int(WIN_W * 0.75), int(WIN_H * 0.35)]
    def load_ghost_pose():
    ...
    def relocate_ghost():
        # 在屏幕安全范围内随机位置
        ghost_origin[0] = random.randint(int(WIN_W * 0.2), int(WIN_W * 0.8))
        ghost_origin[1] = random.randint(int(WIN_H * 0.2), int(WIN_H * 0.7))
    load_ghost_pose()
    relocate_ghost()
    ...
    running = True
    obj = load_collision_object('test1.json')
    obj_x = WIN_W // 2
    obj_y = int(WIN_H * GROUND_Y_RATIO)
    obj_shake = 0.0
    obj_vel = 0.0
    obj_K = 40.0
    obj_C = 12.0
    ...
    while running:
        ...
        if result.multi_hand_landmarks:
            ...
            if calibrate_request_time > 0.0 and time.time() >= calibrate_request_time:
            ...
            if last_wrist is not None:
                ...
                if ALLOW_SHAKE_FLIP and abs(ax_wrist) > SHAKE_ACC_THRESH and (vx * last_wrist_vx < 0) and (now - last_flip_time) > FLIP_COOLDOWN:
                    last_flip_time = now
                    anchors_is_flip = not anchors_is_flip
                    anchors_path = os.path.join(PROJECT_DIR, 'bone', 'anchors_niu_flip.json' if anchors_is_flip else 'anchors_niu.json')
                    sm = SpriteManager(puppet, anchors_path)
                    calibrate_request_time = now + 0.5
                last_wrist_vx = vx
            ...
            def drive_arm_by_rope(side: str, anchor_xy_screen: tuple, finger_id: int = None):
                nonlocal raise_mode_left, raise_mode_right
                ...
                if side == 'left':
                    ...
                    # 方向性增益：抬起/下伸更容易（相同手指范围下更快到顶/更直向下）
            ...
            if neutral_frame_request:
                ...
                try:
                    ...
                    pos_l = b_left_leg.world_pos()
                    tip_l = b_left_leg.tip_offset()
                    pos_r = b_right_leg.world_pos()
                    tip_r = b_right_leg.tip_offset()
                    ly = origin_y + (pos_l.y + tip_l.y)
                    ry = origin_y + (pos_r.y + tip_r.y)
                    on_ground = max(int(ly), int(ry)) >= ground_y
                    if on_ground:
                        body_delta *= ON_GROUND_TILT_GAIN
                except StopIteration:
                    pass
        ...
        for b in bones:
            st = bone_state[b.name]
            tgt = st['target']
            # torsional spring towards target with damping
            # 抬举模式下：小臂使用正常阻尼，上臂维持半阻尼；非抬举模式保持原设定
            ...
            if is_left_raise or is_right_raise:
                ...
                damp = (DAMP_C * 1.0 if b.name in ('left_lower_arm', 'right_lower_arm') else
                        DAMP_C * (0.5 if b.name in ('left_upper_arm', 'right_upper_arm') else 1.0))
        ...
        # Ground: draw a solid band at bottom quarter
        ground_y = int(WIN_H * GROUND_Y_RATIO)
        pygame.draw.rect(screen, GROUND_COLOR, (0, ground_y, WIN_W, WIN_H - ground_y))
        pygame.draw.line(screen, (140, 140, 150), (0, ground_y), (WIN_W, ground_y), 2)
        ...
        if obj:
            ...
            tw = max(1, int(oi.get_width() * sc))
            th = max(1, int(oi.get_height() * sc))
            disp = pygame.transform.smoothscale(oi, (tw, th))
            angle_deg = obj_ang
            rot = pygame.transform.rotate(disp, angle_deg)
            pivot_x = obj_x
            pivot_y = ground_y + int(obj_shake)
            offset = pygame.math.Vector2(0, -th / 2).rotate(-angle_deg)
            rr = rot.get_rect(center=(int(pivot_x + offset.x), int(pivot_y + offset.y)))
            screen.blit(rot, rr)
        ...
        if result and result.multi_hand_landmarks:
            ...
            bone_state, origin_x, origin_y, origin_vx, origin_vy, trunk_waist_drive, trunk_waist_vel, last_ox, last_oy, prev_ovx, prev_ovy = Hand_control_puppet(
                hand, frame, bones, bone_state,
                origin_x, origin_y, origin_vx, origin_vy,
                origin_offset_y, flip,
                last_ox, last_oy, prev_ovx, prev_ovy,
                clock, G,
        ...
        matched = puppet_match(bones, ghost_bones, ghost_origin, origin_x, origin_y, flip, MATCH_THRESH_BODY, MATCH_THRESH_OTHERS, locked_active)
        if matched:
            score += 1
            relocate_ghost()

        if obj:
            obj_shake, obj_vel, obj_ang, obj_ang_vel, obj_ang_target = collide(
                obj, bones, origin_x, origin_y, flip,
                obj_x, ground_y,
                obj_shake, obj_vel, obj_K, obj_C,
                obj_ang, obj_ang_vel, obj_ang_target, obj_ang_K, obj_ang_C,
                OBJ_HIT_ANG,
